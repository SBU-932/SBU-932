...
//TODO: Refine!
public class Gun3 implements GameObject {
	 ...
	 */
	public Gun3(int turn, int x, int y, int r, int l, Color c, Color c2) {
		this.x = x;
		this.y = y;
		this.r = r;
		this.l = l;
		...
		if (Assets.turn[turn]) {
			...
			if (Assets.buttons[2] != shoot) {
				...
				if (shoot && Assets.canShoot) {// If the current key state is
					...
					// Bullet2 b = new Bullet2(x + modf, y + modf, Assets.bSizeW
					// - 1
					// + Assets.boffset, 0.5, angl, b1);
					Bullet3 b = new Bullet3(turn, x + modf, y + modf, r, 0.5, angl,
							b1);
					...
					Assets.canShoot = false;// You can shoot one bullet at a
											// time
					
					//Change turn:
					Assets.turn[turn]= false;
					Assets.turn[Math.abs(turn -1)] = true;
